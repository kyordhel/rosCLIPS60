...
/***************************************************************************
  ...
  NOTES        : WARNING!! - This routine assumes that there are no
                 ...
                 Donald Knuth.
 ***************************************************************************/
globle PACKED_CLASS_LINKS *FindPrecedenceList(cls,supers)
  DEFCLASS *cls;
  PACKED_CLASS_LINKS *supers;
  {
   ...
   if (po_table != NULL)
     {
      PrintErrorID("INHERPSR",5,CLIPS_FALSE);
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle VOID PackClassLinks(plinks,lptop)
  PACKED_CLASS_LINKS *plinks;
  CLASS_LINK *lptop;
  {
...
/**************************************************************************
  ...
  NOTES        : None
 **************************************************************************/
static PARTIAL_ORDER *InitializePartialOrderTable(po_table,supers)
...
/***********************************************************************************
  ...
  NOTES        : None
 ***********************************************************************************/
static VOID RecordPartialOrders(po_table,cls,successors,starti)
  PARTIAL_ORDER *po_table;
  DEFCLASS *cls;
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static PARTIAL_ORDER *FindPartialOrder(po_table,cls)
  PARTIAL_ORDER *po_table;
  DEFCLASS *cls;
  {
   while (po_table != NULL)
 ...
 **************************************************************************/
static VOID PrintPartialOrderLoop(po_table)
  PARTIAL_ORDER *po_table;
  {
   ...
   PrintCLIPS(WERROR,"Precedence loop in superclasses:");
   while (pop1->pre == 1)
     {
      PrintCLIPS(WERROR," ");
      PrintClassName(WERROR,pop1->cls,CLIPS_FALSE);
      pop1->pre = 0;
      pop1 = pop1->suc->po;
     }
   PrintCLIPS(WERROR," ");
   PrintClassName(WERROR,pop1->cls,CLIPS_TRUE);
  }
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static VOID PrintClassLinks(logicalName,title,clink)
   ...
   while (clink != NULL)
     {
      ...
      PrintClassName(logicalName,clink->cls,CLIPS_FALSE);
      clink = clink->nxt;
     }
   PrintCLIPS(logicalName,"\n");
  }
