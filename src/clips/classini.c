...
static ENTITY_RECORD DefclassEntityRecord = { DEFCLASS_PTR,1,0,0,
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
globle VOID ObjectsRunTimeInitialize(ctable,sntable,cidmap,mid)
  DEFCLASS *ctable[];
  SLOT_NAME *sntable[];
  DEFCLASS **cidmap;
  unsigned mid;
...
/***************************************************************
  ...
  NOTES        : The binary/load save indices for the primitive
                   ...
                   classes match their CLIPS integer codes.
...
globle VOID CreateSystemClasses()
  {
   ...
   /* ===================================
      ...
      object patterns
      =================================== */
   AddSlotName(ISA_SYMBOL,ISA_ID,CLIPS_TRUE);
   AddSlotName(NAME_SYMBOL,NAME_ID,CLIPS_TRUE);
...
static VOID SetupDefclasses()
  {
   ...
   DefineFunction2("slot-writablep",'b',PTIF SlotWritablePCommand,"SlotWritablePCommand","22we");
   DefineFunction2("slot-initablep",'b',PTIF SlotInitablePCommand,"SlotInitablePCommand","22we");
   DefineFunction2("slot-publicp",'b',PTIF SlotPublicPCommand,"SlotPublicPCommand","22we");
   DefineFunction2("slot-direct-accessp",'b',PTIF SlotDirectAccessPCommand,
                   "SlotDirectAccessPCommand","22we");
...
/*********************************************************
  ...
  NOTES        : Assumes system-class name is unique
                 ...
                 Adds classes to has table but NOT to
                  ...
                  of caller)
 *********************************************************/
static DEFCLASS *AddSystemClass(name,parent)
  char *name;
  DEFCLASS *parent;
  {
   DEFCLASS *sys;
   ...
   for (i = 0 ; i < CLASS_TABLE_HASH_SIZE ; i++)
     for (theDefclass = ClassTable[i] ;
          ...
          theDefclass = theDefclass->nxtHash)
       {
        ...
        CopyMemory(char,theDefclass->scopeMap->size,
