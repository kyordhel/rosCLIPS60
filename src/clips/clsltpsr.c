 ...
 ************************************************************/
globle TEMP_SLOT_LINK *ParseSlot(readSource,slist,preclist,multiSlot,fieldSpecified)
  char *readSource;
  TEMP_SLOT_LINK *slist;
  PACKED_CLASS_LINKS *preclist;
  int multiSlot,fieldSpecified;
  {
   SLOT_DESC *slot;
   ...
   while (GetType(ObjectParseToken) == LPAREN)
     {
      ...
      if (GetType(ObjectParseToken) != SYMBOL)
        ...
        {
         if (ParseDefaultFacet(readSource,specbits,slot) == CLIPS_FALSE)
           goto ParseSlotError;
        }
      ...
      else if (strcmp(DOToString(ObjectParseToken),CREATE_ACCESSOR_FACET) == 0)
        {
         ...
         if ((rtnCode == 0) || (rtnCode == 2))
           slot->createReadAccessor = CLIPS_TRUE;
         if ((rtnCode == 1) || (rtnCode == 2))
           slot->createWriteAccessor = CLIPS_TRUE;
        }
   ...
   if ((slot->dynamicDefault == 0) && (slot->noWrite == 1) && (slot->initializeOnly == 0))
     slot->shared = 1;
   if (slot->composite)
     BuildCompositeFacets(slot,preclist,specbits,&parsedConstraint);
   if (CheckForFacetConflicts(slot,&parsedConstraint) == CLIPS_FALSE)
     goto ParseSlotError;
   if (CheckConstraintParseConflicts(slot->constraint) == CLIPS_FALSE)
     goto ParseSlotError;
   if (EvaluateSlotDefaultValue(slot,specbits) == CLIPS_FALSE)
     goto ParseSlotError;
...
static SLOT_DESC *NewSlot(name)
  SYMBOL_HN *name;
  {
   ...
   slot->slotName = AddSlotName(name,0,CLIPS_FALSE);
...
/**********************************************************
  ...
  NOTES        : None
 **********************************************************/
static TEMP_SLOT_LINK *InsertSlot(slist,slot)
  TEMP_SLOT_LINK *slist;
  SLOT_DESC *slot;
  {
         ...
         if (stmp->desc->slotName == slot->slotName)
           {
            tmp->nxt = slist;
            DeleteSlots(tmp);
            PrintErrorID("CLSLTPSR",1,CLIPS_FALSE);
            PrintCLIPS(WERROR,"Duplicate slots not allowed.\n");
            return(NULL);
           }
         sprv = stmp;
         stmp = stmp->nxt;
...
/****************************************************************
  ...
  NOTES        : None
 *****************************************************************/
static int ParseSimpleFacet(readSource,specbits,facetName,testBit,
                            clearRelation,setRelation,alternateRelation,
                            varRelation,facetSymbolicValue)
  char *readSource,*specbits,*facetName;
  int testBit;
  char *clearRelation,*setRelation,
       *alternateRelation,*varRelation;
  SYMBOL_HN **facetSymbolicValue;
  {
   ...
   if (TestBitMap(specbits,testBit))
     {
      ...
      PrintCLIPS(WERROR,facetName);
      PrintCLIPS(WERROR," facet already specified.\n");
      return(-1);
     }
   ...
   if (ObjectParseToken.type == SF_VARIABLE)
     {
      if ((varRelation == NULL) ? CLIPS_FALSE :
      ...
      if (facetSymbolicValue == NULL)
        {
         if (strcmp(DOToString(ObjectParseToken),clearRelation) == 0)
           ...
           rtnCode = 1;
         else if ((alternateRelation == NULL) ? CLIPS_FALSE :
