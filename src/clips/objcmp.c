...
/*********************************************************
  ...
  NOTES        : None
 *********************************************************/
globle VOID PrintClassReference(fp,cls,imageID,maxIndices)
  FILE *fp;
  DEFCLASS *cls;
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
globle VOID DefclassCModuleReference(theFile,count,imageID,maxIndices)
  FILE *theFile;
  int count;
  int imageID;
  int maxIndices;
 ...
 ************************************************************/
static VOID MarkDefclassAndSlots(vTheDefclass,vTheBuffer)
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static VOID PrintSlotNameReference(fp,snp,imageID,maxIndices)
  FILE *fp;
  SLOT_NAME *snp;
  int imageID,maxIndices;
  {
...
#pragma argsused
#endif
static VOID InitObjectsCode(initFP,imageID,maxIndices)
  FILE *initFP;
  int imageID;
  int maxIndices;
  {
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static int ObjectsToCode(fileName,fileID,headerFP,imageID,maxIndices)
  char *fileName;
  int fileID;
  FILE *headerFP;
  int imageID;
...
/************************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int ClassIDMapToCode(fileName,fileID,headerFP,imageID,maxIndices,fileCount)
  char *fileName;
  FILE *headerFP;
  int fileID,imageID,maxIndices,*fileCount;
  {
   ...
   classIDMapFile = OpenFileIfNeeded(classIDMapFile,fileName,fileID,imageID,fileCount,
                                     ...
                                     "DEFCLASS *",ClassIDPrefix(),CLIPS_FALSE,NULL);
...
/************************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int ClassHashTableToCode(fileName,fileID,headerFP,imageID,maxIndices,fileCount)
  char *fileName;
  FILE *headerFP;
  int fileID,imageID,maxIndices,*fileCount;
  {
   ...
   classHashFile = OpenFileIfNeeded(classHashFile,fileName,fileID,imageID,fileCount,
                                    ...
                                    "DEFCLASS *",ClassHashPrefix(),CLIPS_FALSE,NULL);
   ...
   classHashFile = CloseFileIfNeeded(classHashFile,&classHashArrayCount,
...
/************************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int SlotNameHashTableToCode(fileName,fileID,headerFP,imageID,maxIndices,fileCount)
  char *fileName;
  FILE *headerFP;
  int fileID,imageID,maxIndices,*fileCount;
  {
   ...
   slotNameHashFile = OpenFileIfNeeded(slotNameHashFile,fileName,fileID,
                                       ...
                                       "SLOT_NAME *",SlotNameHashPrefix(),CLIPS_FALSE,NULL);
   ...
   slotNameHashFile = CloseFileIfNeeded(slotNameHashFile,&slotNameHashArrayCount,
...
/************************************************************
  ...
  NOTES        : None
 ***********************************************************/
static int SlotNameEntriesToCode(fileName,fileID,headerFP,imageID,maxIndices,fileCount)
  char *fileName;
  FILE *headerFP;
  int fileID,imageID,maxIndices,*fileCount;
  {
      ...
      for (snp = SlotNameTable[i] ; snp != NULL ; snp = snp->nxt)
        {
         slotNameFile = OpenFileIfNeeded(slotNameFile,fileName,fileID,
                                       ...
                                       "SLOT_NAME",SlotNamePrefix(),CLIPS_FALSE,NULL);
...
/******************************************************
  ...
  NOTES        : None
 *****************************************************/
static VOID CloseObjectFiles(itemFiles,itemReopenFlags,itemCodeFiles,maxIndices)
  FILE *itemFiles[SAVE_ITEMS];
  int itemReopenFlags[SAVE_ITEMS];
  struct CodeGeneratorFile itemCodeFiles[SAVE_ITEMS]; 
  int maxIndices;  
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static VOID DefclassModuleToCode(theFile,theModule,imageID,maxIndices)
  FILE *theFile;
  struct defmodule *theModule;
  int imageID;
  int maxIndices;
...
/****************************************************************
  ...
  NOTES        : None
 ***************************************************************/
static VOID SingleDefclassToCode(theFile,imageID,maxIndices,
                                 theDefclass,moduleCount,
                                 classLinkArrayVersion,classLinkArrayCount,
  ...
  DEFCLASS *theDefclass;
  int moduleCount;
  int classLinkArrayVersion,classLinkArrayCount,
      slotArrayVersion,slotArrayCount,
      templateSlotArrayVersion,templateSlotArrayCount,
      orderedSlotArrayVersion,orderedSlotArrayCount,
      handlerArrayVersion,handlerArrayCount,
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static BOOLEAN InheritanceLinksToCode(classLinkFile,fileName,fileID,imageID,
                                      headerFP,fileCount,maxIndices,theDefclass,
                                      classLinkArrayVersion,classLinkArrayCount,
                                      reopenClassLinkFile,classLinkCodeFile)
  FILE **classLinkFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *classLinkArrayVersion,*classLinkArrayCount,*reopenClassLinkFile;
  struct CodeGeneratorFile *classLinkCodeFile;
  {
   register unsigned i;
   int inheritanceLinkCount,
       linkPrinted = CLIPS_FALSE;
...
/***********************************************************
  ...
  NOTES        : None
 ***********************************************************/
static BOOLEAN SlotsToCode(slotFile,fileName,fileID,imageID,
                           headerFP,fileCount,maxIndices,theDefclass,
                           slotArrayVersion,slotArrayCount,
                           reopenSlotFile,slotCodeFile)
  FILE **slotFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *slotArrayVersion,*slotArrayCount,*reopenSlotFile;
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static BOOLEAN TemplateSlotsToCode(templateSlotFile,fileName,fileID,imageID,
                                   headerFP,fileCount,maxIndices,theDefclass,
                                   templateSlotArrayVersion,templateSlotArrayCount,
                                   reopenTemplateSlotFile,templateSlotCodeFile)
  FILE **templateSlotFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *templateSlotArrayVersion,*templateSlotArrayCount,*reopenTemplateSlotFile;
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static BOOLEAN OrderedSlotsToCode(orderedSlotFile,fileName,fileID,imageID,
                                  headerFP,fileCount,maxIndices,theDefclass,
                                  orderedSlotArrayVersion,orderedSlotArrayCount,
                                  reopenOrderedSlotFile,orderedSlotCodeFile)
  FILE **orderedSlotFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *orderedSlotArrayVersion,*orderedSlotArrayCount,*reopenOrderedSlotFile;
...
/*************************************************************
  ...
  NOTES        : None
 *************************************************************/
static BOOLEAN HandlersToCode(handlerFile,fileName,fileID,imageID,
                              headerFP,fileCount,maxIndices,theDefclass,
                              handlerArrayVersion,handlerArrayCount,
                              reopenHandlerFile,handlerCodeFile)
  FILE **handlerFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *handlerArrayVersion,*handlerArrayCount,*reopenHandlerFile;
...
/****************************************************************
  ...
  NOTES        : None
 ****************************************************************/
static BOOLEAN OrderedHandlersToCode(orderedHandlerFile,fileName,fileID,imageID,
                                     headerFP,fileCount,maxIndices,theDefclass,
                                     orderedHandlerArrayVersion,orderedHandlerArrayCount,
                                     reopenOrderedHandlerFile,orderedHandlerCodeFile)
  FILE **orderedHandlerFile,*headerFP;
  char *fileName;
  int fileID,imageID,*fileCount,maxIndices;
  DEFCLASS *theDefclass;
  int *orderedHandlerArrayVersion,*orderedHandlerArrayCount,*reopenOrderedHandlerFile;
