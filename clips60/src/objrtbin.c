...
static OBJECT_ALPHA_NODE HUGE_ADDR *AlphaArray = NULL;
static OBJECT_PATTERN_NODE HUGE_ADDR *PatternArray = NULL;
...
static VOID BsaveObjectPatternsFind()
  {
   ...
   while (alphaPtr != NULL)
     {
      alphaPtr->classbmp->neededBitMap = CLIPS_TRUE;
      if (alphaPtr->slotbmp != NULL)
        alphaPtr->slotbmp->neededBitMap = CLIPS_TRUE;
...
/****************************************************
  ...
  NOTES        : None
 ****************************************************/
static VOID BsaveStorageObjectPatterns(fp)
  FILE *fp;
  {
   UNLN space;
 ...
 ***************************************************/
static VOID BsaveObjectPatterns(fp)
  FILE *fp;
  {
   UNLN space;
   OBJECT_ALPHA_NODE *alphaPtr;
...
static VOID BloadStorageObjectPatterns()
  {
   ...
   if (AlphaNodeCount == 0L)
   ...
   else
     {
      ...
      AlphaArray = (OBJECT_ALPHA_NODE HUGE_ADDR *) genlongalloc(space);
     }
   if (PatternNodeCount == 0L)
   ...
   else
     {
      ...
      PatternArray = (OBJECT_PATTERN_NODE HUGE_ADDR *) genlongalloc(space);
     }
  }
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static VOID UpdateAlpha(buf,obji)
  VOID *buf;
  long obji;
  {
...
/***************************************************
  ...
  NOTES        : None
 ***************************************************/
static VOID UpdatePattern(buf,obji)
  VOID *buf;
  long obji;
  {
   ...
   op = &PatternArray[obji];
   
   op->blocked = CLIPS_FALSE;
