 ...
 */
globle int addline(bufferp,text)
BUFFER *bufferp;
char    *text;
{
        register LINE   *lp;
 ...
 */
globle BUFFER  *bfind(bname, cflag, bflag)
char   *bname;
int cflag,bflag;
{
        register BUFFER *bp;
 ...
 */
globle VOID updateline(row, vline, pline)
    int row;
    char vline[];
    char pline[];
{
 ...
 */
globle VOID movecursor(row, col)
int row,col;
    {
 ...
 */
globle int mlreply(prompt, buf, nbuf)
    char *prompt;
    char *buf;
    int nbuf;
    {
...
/*
  You can only pass mlwrite a maximum of 5 arguments reliably
*/
globle VOID mlwrite(fmt,arg1,arg2,arg3,arg4,arg5)
    char *fmt;
...
static VOID int_to_ascii(buf, width, num)
char   buf[];
int    width;
int    num;
{
